      ...
      <v-col cols="12" sm="6" md="3">
        ...
        >
          <div class="d-flex flex-column justify-center h-100">
            <div class="text-body-2 text-white mb-4">Mis Documentos</div>
      ...
      <v-col cols="12" md="9">
        ...
        <v-card>
          <v-card-title>
            ...
            {{ carpetaSeleccionada ? carpetaSeleccionada.nombre : 'Mis Documentos' }}
  ...
  methods: {
    ...
    actualizarContadoresCarpetas() {
      ...
      this.carpetas.forEach(carpeta => {
        const categoria = this.getCategoriaFromCarpeta(carpeta.id)
        carpeta.documentos = this.documentos.filter(doc => doc.categoria === categoria).length
      })
    },

    getCategoriaFromCarpeta(carpetaId) {
      const mapping = {
        1: 'Facturas',
        2: 'Comprobantes',
        3: 'Contratos',
      ...
      return mapping[carpetaId] || 'Otros'
    },
        ...
        try {
          ...
          for (let i = 0; i < this.archivosSeleccionados.length; i++) {
            ...
            try {
              ...
              // Usar el ID del usuario actual (el servicio ya maneja el client_id automáticamente)
              const documentData = {
                ...
                uploadedBy: this.currentUserId // Usar el ID del usuario actual
              }
