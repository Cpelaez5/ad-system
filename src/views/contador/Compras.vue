<template>
  <v-container fluid class="pa-4">
    ...
    <v-row class="mb-4" v-if="canFilterByClient">
      <v-col cols="12" md="4">
        <v-select
          ...
          item-value="id"
          label="Filtrar por Proveedor/Cliente"
          variant="outlined"
          clearable
    ...
    <v-card>
      <v-card-title>
        ...
        Compras ({{ filteredInvoices.length }})
      </v-card-title>
      <v-data-table :headers="headers" :items="filteredInvoices" :loading="loading" class="elevation-1" :items-per-page="10">
        <template v-slot:item.invoiceNumber="{ item }">
          <v-chip color="secondary" variant="tonal">{{ item.invoiceNumber }}</v-chip>
        </template>
        <template v-slot:item.client="{ item }">
          <div>
            <div class="font-weight-medium">{{ item.issuer?.companyName || item.client_info?.companyName || 'N/A' }}</div>
            <div class="text-caption text-grey">{{ item.issuer?.rif || item.client_info?.rif || '' }}</div>
          </div>
        </template>
    ...
    <v-dialog v-model="invoiceDialog" max-width="1200px" scrollable>
      <InvoiceForm :invoice="editingInvoice" :flow="'COMPRA'" :auto-party-mode="true" @submit="handleInvoiceSubmit" @cancel="closeInvoiceDialog" />
    </v-dialog>

    <v-dialog v-model="deleteDialog" max-width="400px">
      <v-card>
        <v-card-title>Confirmar Eliminación</v-card-title>
        <v-card-text>¿Eliminar la compra <strong>{{ invoiceToDelete?.invoiceNumber }}</strong>?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  async mounted() {
    await this.loadUserAndClients()
    await this.reloadData()
  },
  methods: {
    async loadUserAndClients() {
      try {
        ...
        // Los contadores y admins pueden filtrar por clientes de su organización
        this.canFilterByClient = true
        console.log('🔄 Cargando clientes de la organización para filtrado...')
        this.clients = await clientService.getClients()
        console.log('✅ Clientes cargados para filtrado:', this.clients.length)
      } catch (e) {
        console.error('❌ Error al cargar usuario y clientes:', e)
        this.clients = []
      } finally {
    ...
    async loadInvoices() {
      this.loading = true
      try {
        console.log('🔄 Cargando compras (facturas de COMPRA) de la organización...')
        // Las facturas se filtran automáticamente por organization_id en el servicio
        // Si hay un cliente seleccionado, filtrar también por client_id
        const resp = await invoiceService.getInvoices({ 
          flow: 'COMPRA', 
          clientId: this.selectedClientId || undefined 
        })
        this.invoices = resp || []
        console.log('✅ Compras cargadas:', this.invoices.length)
        this.computeStats()
      } catch (e) {
    ...
    applyFilters() {
      ...
      if (this.searchQuery) {
        ...
        filtered = filtered.filter(i =>
          ...
          (i.issuer?.companyName || i.client_info?.companyName || '').toLowerCase().includes(s)
        )
      }
    ...
    editInvoice(inv) {
      this.editingInvoice = { ...inv, flow: 'COMPRA' }
      this.invoiceDialog = true
    },
    viewInvoice(inv) {
