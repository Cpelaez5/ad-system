  ...
  <v-container fluid class="pa-4">
      ...
      <v-col cols="12" sm="6" md="3">
        <v-card class="pa-6 stats-card" height="120" style="background-color: #02254d;">
          <div class="d-flex flex-column justify-center h-100">
            <div class="text-body-2 text-white mb-4">Total Compras</div>
    ...
    <v-row class="mb-4">
      <v-col cols="12" md="6">
        <v-text-field
          ...
          label="Buscar factura de compra de la organización..."
          variant="outlined"
          clearable
          hide-details
          @input="applyFilters"
    ...
    <v-card>
      <v-card-title>
        ...
        Compras de la Organización ({{ filteredInvoices.length }})
      </v-card-title>
      <v-card-subtitle class="text-caption">
        Mercancía y productos: hojas, carpetas, suministros de oficina, etc.
      </v-card-subtitle>
    ...
    <v-dialog v-model="invoiceDialog" max-width="1200px" scrollable>
      <InvoiceForm :invoice="editingInvoice" :flow="'COMPRA'" :auto-party-mode="true" :organizationOnly="true" @submit="handleInvoiceSubmit" @cancel="closeInvoiceDialog" />
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>
          ¿Eliminar la compra <strong>{{ invoiceToDelete?.invoiceNumber }}</strong>?
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { title: 'Proveedor', key: 'provider', sortable: true },
  ...
  methods: {
    ...
    async loadInvoices() {
      this.loading = true
      try {
        console.log('🔄 Cargando compras de la organización (sin client_id)...')
        // Cargar facturas de COMPRA de la organización (client_id IS NULL)
        const resp = await invoiceService.getInvoices({ 
          flow: 'COMPRA', 
          organizationOnly: true 
        })
        this.invoices = resp || []
        console.log('✅ Compras de organización cargadas:', this.invoices.length)
        this.computeStats()
      } catch (e) {
        console.error('❌ Error cargando compras de organización:', e)
        this.invoices = []
        this.computeStats()
      } finally {
        this.loading = false
    ...
    editInvoice(inv) {
      this.editingInvoice = { ...inv, flow: 'COMPRA', organizationOnly: true }
      this.invoiceDialog = true
    },
    viewInvoice(inv) {
