  ...
  <v-container fluid class="pa-4">
      ...
      <v-col cols="12" sm="6" md="3">
        <v-card class="pa-6 stats-card" height="120" style="background-color: #02254d;">
          <div class="d-flex flex-column justify-center h-100">
            <div class="text-body-2 text-white mb-4">Total Gastos</div>
    ...
    <v-row class="mb-4">
      <v-col cols="12" md="6">
        <v-text-field
          ...
          label="Buscar factura de gasto de la organización..."
          variant="outlined"
          clearable
          hide-details
          @input="applyFilters"
    ...
    <v-card>
      <v-card-title>
        ...
        Gastos de la Organización ({{ filteredInvoices.length }})
      </v-card-title>
      <v-card-subtitle class="text-caption">
        Servicios constantes: internet, facturas de servicios, etc.
      </v-card-subtitle>
      <v-data-table :headers="headers" :items="filteredInvoices" :loading="loading" class="elevation-1" :items-per-page="10">
        <template v-slot:item.invoiceNumber="{ item }">
          <v-chip color="error" variant="tonal">{{ item.invoiceNumber }}</v-chip>
        </template>
    ...
    <v-dialog v-model="deleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>
          ¿Eliminar el gasto <strong>{{ invoiceToDelete?.invoiceNumber }}</strong>?
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    async loadInvoices() {
      this.loading = true
      try {
        console.log('🔄 Cargando gastos de la organización (sin client_id)...')
        ...
        this.invoices = resp || []
        console.log('✅ Gastos de organización cargados:', this.invoices.length)
        this.computeStats()
      } catch (e) {
        console.error('❌ Error cargando gastos de organización:', e)
        this.invoices = []
        this.computeStats()
      } finally {
        this.loading = false
