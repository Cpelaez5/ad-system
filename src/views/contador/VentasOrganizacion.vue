  ...
  <v-container fluid class="pa-4">
    ...
    <v-row class="mb-4">
      <v-col cols="12" md="6">
        <v-text-field
          ...
          label="Buscar factura de venta de la organización..."
          variant="outlined"
          clearable
          hide-details
          @input="applyFilters"
    ...
    <v-card>
      <v-card-title>
        ...
        Ventas de la Organización ({{ filteredInvoices.length }})
      </v-card-title>
      <v-card-subtitle class="text-caption">
        Ingresos de la organización por servicios prestados
      </v-card-subtitle>
    ...
    <v-dialog v-model="invoiceDialog" max-width="1200px" scrollable>
      <InvoiceForm :invoice="editingInvoice" :flow="'VENTA'" :auto-party-mode="true" :organizationOnly="true" @submit="handleInvoiceSubmit" @cancel="closeInvoiceDialog" />
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async loadInvoices() {
      this.loading = true
      try {
        console.log('🔄 Cargando ventas de la organización (sin client_id)...')
        // Cargar facturas de VENTA de la organización (client_id IS NULL)
        const resp = await invoiceService.getInvoices({ 
          flow: 'VENTA', 
          organizationOnly: true 
        })
        this.invoices = resp || []
        console.log('✅ Ventas de organización cargadas:', this.invoices.length)
        this.computeStats()
      } catch (e) {
        console.error('❌ Error cargando ventas de organización:', e)
        this.invoices = []
        this.computeStats()
      } finally {
        this.loading = false
    ...
    editInvoice(inv) {
      this.editingInvoice = { ...inv, flow: 'VENTA', organizationOnly: true }
      this.invoiceDialog = true
    },
    viewInvoice(inv) {
